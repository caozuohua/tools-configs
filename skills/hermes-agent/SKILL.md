...
### Skill will not load (`skill_view` returns "not found" but `skills_list` has it)
Observed 2026-06-27: `skill_view(name='gcp-vps-ops')` returned `not found`, `skill_manage(action='patch')` also returned `not found`, yet `skills_list(category='devops')` confirmed the skill existed. A subsequent retry of `skill_view` succeeded.
**Root cause**: `skill_view`'s first invocation may fail transiently (cache/index lag), and the error message ("not found") is ambiguous — it does not distinguish "does not exist" from "failed to read right now".
**Reliable diagnosis workflow**:
1. `skills_list` first — confirm the skill name exists in the catalog.
2. If `skills_list` shows it but `skill_view` says "not found": call `skill_view` once more (one retry is enough). If still failing, check `skill_view` without the `category/` prefix (the internal name may differ from the catalog display name).
3. If `skills_list` does NOT show it either: the skill genuinely does not exist (or is not installed for the current profile). Use `hermes skills install <name>` to install it.
