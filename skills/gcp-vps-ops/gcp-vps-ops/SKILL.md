...
SSH troubleshooting playbook above). See `references/ssh-connection-diagnosis.md` for the systematic diagnosis playbook when SSH fails completely — covers key rejection, gcloud IP-vs-name errors, sshpass fallback, and non-GCP VPS (Hostwinds/Vultr) access patterns.
...
- **gcloud SSH requires instance NAME, not IP** — `gcloud compute ssh <IP>` fails with `Must be a match of regex`. You must use the instance name (e.g., `instance-20260413-080555`). If you don't know the name, use `gcloud compute instances list --project=<PROJECT>` (requires `compute.instances.list` permission). For non-GCP VPS (Hostwinds, Vultr, DigitalOcean, etc.), gcloud SSH is not an option — use `sshpass` or the provider's web console.
- **gcloud SSH via public IP + --region + --network** — When you don't have instance name (e.g., service account lacks `compute.instances.list`), you CAN use the public IP with `--region` and `--network` flags: `gcloud compute ssh root@<IP> --region=us-central1 --network=default --project=<PROJECT> --zone=<ZONE>`. This bypasses the instance-name requirement and IAP tunnel. **gcloud CLI v473+ forces IAP tunnel for public IPs even with `--plain`** — the `--region + --network` approach is the reliable bypass.
- **Wrong project ID** — `gcloud config get-value project` may not be the project with VMs. The GCP auto-created service account (`PROJECT_NUMBER-compute@developer.gserviceaccount.com`) often has NO compute permissions. Always run `gcloud projects list` first, then try listing instances in each project. The actual project may have a different ID format (e.g., `project-c1ed131b-6f02-49de-9f8` vs `gen-lang-client-0814665573`).
- **Xray managed by 3x-ui has no systemd service** — When xray is managed by the 3x-ui panel, it runs as a child process of x-ui, NOT as a separate systemd service. `systemctl is-active xray` returns `inactive` even when the proxy is working perfectly. Check with `ps aux | grep xray` instead.
